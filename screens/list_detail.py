...
class ListDetailScreen(BaseScreen):
    def __init__(self, screen, manager, title, items, on_select=None):
        super().__init__(screen)
        self.manager = manager
        self.title = title
        # items: list of dict { "id":..., "label":..., "image": "path", "lines": [...] }
        self.items = items
        ...
        # create touch areas for list entries (optional)
        self.touch_areas = []
    ...
    def render(self):
        ...
        # title
        title_s = glow_title(self.title, self.font_title)
        ...
        for i, it in enumerate(self.items):
            ...
            surf = glow_text(it["label"], self.font_item) if is_sel else glow_text(it["label"], self.font_item)
